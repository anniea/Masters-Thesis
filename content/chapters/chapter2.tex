...
\subsection{Saliency Maps} % MIKAL
...
\noindent A saliency map would typically be computed for an input image and its resulting top classification. To obtain the saliency map, backpropagation must be employed to find the gradient of the loss of the selected class with respect to the image. This gradient will be a matrix with the same dimensions as the input image. The absolute value of the gradient matrix should then be calculated and the entries scaled to match standard pixel values. If the input is an RGB image, the matrix should be transformed into 2D by computing the maximum values across the color channels. The resulting matrix corresponds to a greyscale image that represents the saliency map of the input for the chosen class. The map and input have equal size in the spatial dimensions, and the map entry at \textit{(i,j)} shows the saliency of the input pixel at \textit{(i,j)}, RGB or otherwise. Note that it is also possible to compute the positive saliency, i.e. the pixels that are positively correlated with the output, by discarding the negative gradient values instead of computing their absolute. The negative saliency map can be acquired in a similar manner by discarding the positive gradient values. \\


\begin{comment} % annie, gammelt
\noindent A saliency map would typically be computed for an input image and its resulting top classification. To obtain the saliency map, first the gradient of the loss with respect to the image is found by backpropagation. The resulting gradient will be a 2D vector with the same dimensions as the input image. The absolute value of the gradient is calculated, and the entries are scaled to fit between 0 and 255. An entry in the \textit{i}-th row and \textit{j}-th column of the gradient vector corresponds to the pixel \textit{(i,j)} in the input image and thus pixel \textit{(i,j)} in the saliency map as well. If the image is an RGB image, the single saliency value for a pixel will be the maximum of the gradient across the color channels. The saliency map will be constructed using these values. Note that it is also possible to compute the positive saliency, i.e. the pixels that are positively correlated with the output, by discarding the negative gradient values instead of taking the absolute value. The negative saliency can be computed in a similar manner by discarding the positive gradient values. \\
\end{comment}

\begin{comment} % fra høstprosjektet (?)
A salience map is typically a black-and-white image with the same size as the input, where important pixels have a high value, and insignificant pixels have a low value \cite{salience}. Thus, the important regions can be easily identified by their brightness. A map for classification is specific to an image and a class, and it allows us to observe which parts of an image influences the network classification score the most. In other words, we can view which parts of an image that the network deems more important when deciding upon the classification score chosen. Such a map can be produced by efficiently using the back-propagation gradient of the output score with respect to the input image. \\

\noindent In a face recognition case, the saliency would allow us to investigate what the network emphasizes, for example which facial regions are decisive in producing the identification output. We could also exploit salience maps for expression classification, creating maps to display which areas were the most influential for selecting the outputted expression results. By using the insight gained through salience maps, we would be able to confirm our beliefs that a network focuses on regions containing important feature information, and alert us to any peculiarities.
\end{comment}
...
\section{Transfer Learning} % MIKAL
...
% not sure if there should be subsections or just one large section.
\begin{comment}
\noindent There are two primary approaches for employing transfer learning:

\begin{itemize}
    \item Using the CNN as a fixed feature extractor
    \item Fine-tuning the CNN
\end{itemize}
\end{comment}
...
\textit{Use stuff from Specialization Project.}
