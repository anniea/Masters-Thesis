...
A variation of the traditional ANN, convolutional neural networks (CNNs) are especially well suited for handling input data that can be arranged in a grid-like topology. These types of networks are commonly used in situations where the processing of 2D-images is required, but can be applied to input of any dimension. CNNs employ a mathematical operation called convolution, from which the name is derived. 

Explain convnet structure, that it includes pooling layers and usually fully connected layers at top.

In addition to layers implementing this operation, CNNs typically contain pooling layers. Before these two layer types, as well as the transposed convolution layer, are explained further, we review the convolution operation.
