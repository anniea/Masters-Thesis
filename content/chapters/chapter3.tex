...
Although we do not aspire to explicitly create a network that pushes the state of the art, we are interested in investigating a technique that might prove useful in future attempts to do so. This section therefore introduces a selection of state of the art systems for facial recognition. 

advantageous 


such networks are still of interest.  as they represent the networks we ultimately want to improve. 

if the proposed approach proves advantageous, the goal would be to implement it in a similar manner in a state of the art network. 



As mentioned, the proposed approach of exploiting facial expression in an face recognition network is a novel one. While this network will be experimental, we still need a baseline network to use as a starting point and to measure performance against. 

we review such networks here as their structure
