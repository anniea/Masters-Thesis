...
% mention that baseline got 100 % training here
...
% fra case study networks. må skrives litt om
...
\end{tabular}
\end{center}
\caption[Extra Input architectures]{Extra Input network architectures. Note that the layer type refers to the Keras 2 API. All Dense (fully connected) layers use ReLU activation functions, except the output layer of every configuration, which use softmax. All Dropout layers use a 0.5 dropout rate. The Concatenate layer is used to add the additional input, by concatenating the output from the previous layer with the expression input vector.}
...
% why bad
% sometimes uses extra heavy LR

\subsubsection{Training}

% all networks trained on same hardware (list hardware)
% list training times
% training hyperparameters
% describe use of features over images, why and how
...
\multirow{9}{*}{H} & 0 & input & InputLayer & \multirow{2}{*}{4096} \\ \cline{2-4}
& 1 & drop1 & Dropout & \\ \cline{2-5}
& 2 & fc1 & Dense & \multirow{4}{*}{1024} \\ \cline{2-4}
& 3 & drop2 & Dropout & \\ \cline{2-4}
& 4 & fc2 & Dense & \\ \cline{2-4}
...
& 5 & drop3 & Dropout & \\ \cline{2-5}
& 6-1 & id\_output & \multirow{2}{*}{Dense} & 98 \\ \cline{2-3} \cline{5-5}
& 6-2 & fc2 & & \multirow{2}{*}{1024} \\ \cline{2-4}
& 7-2 & drop2 & Dropout & \\ \cline{2-5}
 & 8-2 & exp\_output & Dense & 7 \\ \cline{2-5}
 & 9 & concat & Concatenate & 105 \\ \hline
\end{tabular}
\end{center}
\caption[Extra Output architectures]{Extra Output network architectures. Note that the layer type refers to the Keras 2 API. All Dense (fully connected) layers use ReLU activation functions, except the output layer of every configuration, which use softmax. All Dropout layers use a 0.5 dropout rate. The Concatenate layer is used to add the addition input, by concatenating the output from the previous layer with the expression input vector.}
